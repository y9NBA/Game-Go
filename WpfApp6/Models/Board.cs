...
namespace Go.Models
{
    public class Board: ViewModel
    {
        ...
        {
            if (field[x, y].CurrentState != 0) return false;
            bool captured = false;
            foreach(Point n in GetNeighbors(x, y))
            {
                if (field[n.X,n.Y].CurrentState == OppositeColor(color) && IsCapture(n.X,n.Y, color))
                {
                    captured = true; break;
                }
            }
